...
export function InvoiceModal({ modal, setModal }) {
  ...
  useEffect(() => {
    ...
    async function getModalData() {
      const obj = {
        columns: "a.path, m.name,p.receiptNo",
        tableName: "tblAttachment a",
        joins: `LEFT JOIN tblMasterData m ON m.id = a.attachmentTypeId 
                left Join tblInvoicePayment p on p.id=a.invoicePaymentId
`,
        whereCondition: `a.invoicePaymentId in (${modal.value}) AND a.status = 1 and m.name='Receipt'`,
      };
