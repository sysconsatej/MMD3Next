...
import { ContainerStatus } from "../containerStatusData";
...
export default function ContainerStatusList() {
  ...
  const { data } = useGetUserAccessUtils("Container Status");
    ...
    async (pageNo = page, pageSize = rowsPerPage) => {
      try {
        const tableObj = {
          ...
          joins: ` join tblMasterData m1 on m1.id = m.id and m.masterListName = 'tblContainerStatus' left join tblUser u on u.id = m.updatedBy`,
        };
  ...
  const modeHandler = (mode, formId = null) => {
    ...
    setMode({ mode, formId });
    router.push("/master/containerStatus");
  };
      ...
      <Box className="sm:px-4 py-1 ">
        <Box className="flex flex-col sm:flex-row justify-between pb-1">
          ...
          <Box className="flex flex-col sm:flex-row gap-6">
            ...
            />
            <CustomButton text="Add" href="/master/containerStatus" />
          </Box>
        </Box>
