...
import { ContainerType } from "../containerTypeData";
...
export default function ContainerTypeList() {
  ...
  const { data } = useGetUserAccessUtils("Container Type");
    ...
    async (pageNo = page, pageSize = rowsPerPage) => {
      try {
        const tableObj = {
          ...
          joins: ` join tblMasterData m1 on m1.id = m.id and m.masterListName = 'tblType' left join tblUser u on u.id = m.updatedBy`,
        };
      ...
      <Box className="sm:px-4 py-1 ">
        <Box className="flex flex-col sm:flex-row justify-between pb-1">
          ...
          <Box className="flex flex-col sm:flex-row gap-6">
            ...
            />
            <CustomButton text="Add" href="/master/containerType" />
          </Box>
        </Box>
