...
import { movementType } from "../movementTypeData";
...
export default function MovementTypeList() {
  ...
  const { data } = useGetUserAccessUtils("Movement Type");
    ...
    async (pageNo = page, pageSize = rowsPerPage) => {
      try {
        const tableObj = {
          ...
          joins: ` join tblMasterData m1 on m1.id = m.id and m.masterListName = 'tblMovementType' left join tblUser u on u.id = m.updatedBy`,
        };
        const { data, totalPage, totalRows } = await fetchTableValues(tableObj);
        setMovementTypeData(data);
  ...
  const modeHandler = (mode, formId = null) => {
    ...
    setMode({ mode, formId });
    router.push("/master/movementType");
  };
      ...
      <Box className="sm:px-4 py-1 ">
        <Box className="flex flex-col sm:flex-row justify-between pb-1">
          ...
          <Box className="flex flex-col sm:flex-row gap-6">
            ...
            />
            <CustomButton text="Add" href="/master/movementType" />
          </Box>
        </Box>
