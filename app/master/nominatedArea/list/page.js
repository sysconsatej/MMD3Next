...
export default function NominatedAreaList() {
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [totalPage, setTotalPage] = useState(1);
  const [totalRows, setTotalRows] = useState(1);
  const [nominatedAreaData, setNominatedAreaData] = useState([]);
  ...
  const rows = nominatedAreaData
    ? nominatedAreaData.map((item) =>
        createData(
          item["code"],
          item["description"],
          item["address"],
        ...
        <Box className="flex flex-col sm:flex-row justify-between pb-1">
          ...
          <Box className="flex flex-col sm:flex-row gap-6">
            <SearchBar
              ...
              search={search}
              setSearch={setSearch}
              options={dropdowns.nominatedArea}
            />
            <CustomButton text="Add" href="/master/nominatedArea" />
          </Box>
        </Box>
