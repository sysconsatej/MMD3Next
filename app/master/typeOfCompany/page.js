...
import data from "./typeOfCompanyData";
  ...
  const handleBlurEventFunctions = {
    duplicateHandler: async (event) => {
      ...
      const obj = {
        ...
        whereCondition: ` ${name} = '${value}' and masterListName = 'tblSubtypeCompany'  and status = 1`,
      };
  ...
  useEffect(() => {
    async function initialHandler() {
      const obj = {
        ...
        whereCondition: "name = 'tblSubtypeCompany'",
      };
        ...
        <section className="py-1 px-4">
          ...
          <Box className="border border-solid border-black rounded-[4px] ">
            <Box className="sm:grid sm:grid-cols-3 gap-2 flex flex-col p-1 border-b border-b-solid border-b-black ">
              <CustomInput
                fields={jsonData.companyTypeFields}
                formData={formData}
