...
import data, { metaData } from "./onlineCheckCsnCustomerData";
...
export default function CSN() {
  ...
  const handleCheck = async () => {
    try {
      ...
      const ids = tableFormData.map((row) => row.ID).filter(Boolean);

      const body = {
        spName: "preCsnVerificationCustomer",
        jsonData: { ids },
      };

      const res = await fetchDynamicReportData(body);

      const rows = res?.data?.data || [];

      const formatted = rows.map((item, index) => ({
        id: index + 1,
        referenceNo: item.blNo || "-",
        message: item.msg || item.message || "",
      }));

      setErrorGrid(formatted);
      setShowErrors(true);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    const requestBody = {
      spName: "ImportMBlSelectionCustomer",
      jsonData: {
        ...transformed,
    ...
    <ThemeProvider theme={theme}>
      <form>
        <section className="py-1 px-4">
          ...
          <Box className="w-full flex mt-2  gap-2">
            ...
            />
            <CustomButton
              text={checkLoading ? "Checking..." : "CHECK"}
              onClick={handleCheck}
      ...
      <Box className="p-0">
        {!showErrors ? (
          ...
          <ErrorList
            errorGrid={errorGrid}
            fileName={"CSN-Verification-Errors"}
          />
        )}
      </Box>
      <ToastContainer />
