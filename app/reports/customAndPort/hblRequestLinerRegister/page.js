...
import data, { metaData } from "./hblRequestLinerRegisterData";
      ...
      <form onSubmit={handleSubmit}>
        <section className="py-1 px-4">
          <Box className="flex justify-between items-end py-1">
            <h1 className="text-left text-base m-0">HBL Request Liner Register</h1>
          </Box>

          <Box className="border border-black rounded-[4px]">
            <Box className="sm:grid sm:grid-cols-5 gap-2 flex flex-col p-1 border-b border-black">
              <CustomInput
                fields={data.igmGenerationFields}
                formData={formData}
          ...
          <Box className="w-full flex mt-2 gap-2">
            ...
            <DynamicReportDownloadExcelButton
              rows={tableFormData}
              metaData={metaData}
              fileName={`HblRequestLinerReport_${new Date()
                .toISOString()
                .slice(0, 10)}.xlsx`}
              text="DOWNLOAD EXCEL"
