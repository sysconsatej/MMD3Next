...
export default function SlabWiseReport() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const requestBody = {
        spName: "slabWiseReport", // Replace with your actual SP name
        jsonData: {
          ...formattedForApi,
      ...
      <form onSubmit={handleSubmit}>
        <section className="py-1 px-4">
          <Box className="flex justify-between items-end py-1">
            <h1 className="text-left text-base m-0">Slab Wise Report</h1>
          </Box>
          ...
          <Box className="border border-black rounded-[4px]">
            <Box className="sm:grid sm:grid-cols-3 gap-2 flex flex-col p-1 border-b border-black">
              <CustomInput
                fields={data.slabWiseReportFields}
                formData={formData}
          ...
          <Box className="w-full flex mt-2 gap-2">
            ...
            <DynamicReportDownloadExcelButton
              rows={tableFormData}
              metaData={metaData}
              fileName={`SlabWiseReport_${new Date()
                .toISOString()
                .slice(0, 10)}.xlsx`}
              text="DOWNLOAD EXCEL"
